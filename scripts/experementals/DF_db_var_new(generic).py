...
def build_queries(conn):
    ...
    # --- Временная таблица с регулярными выражениями ---
    tr = Table('tmp_token_regex', MetaData(), autoload_with=conn)

    # LATERAL: поиск совпадений форм слова в каждом предложении
    rm_lateral = lateral(
        func.regexp_matches(
    ...
    # Подсчёт количества совпадений на предложение
    matches_lateral = (
        select(
        ...
        .select_from(
            join(s, tr, tr.c.token_text == tok.Token_text)
    ...
    # --- total_token_count: сумма вхождений всех форм слова
    token_freq_query = (
        select(
            tok.Token_text.label('Token_text'),
            func.coalesce(func.sum(matches_lateral.c.cnt), 0).label('total_token_count')
        )
        .outerjoin(matches_lateral, true())
        .group_by(tok.Token_text)
    )

    # --- sentence_count: число предложений с формами токена
    ...
    # --- dependent_word_count: количество зависимых слов для токена
    dependent_word_count_query = (
        select(
            tok.Token_text.label('Token_text'),
            func.count(distinct(w.WordID)).label('dependent_word_count')
        )
        .join(w, w.TokenID == tok.TokenID)
        .group_by(tok.Token_text)
    )

    # --- pos_dependency: зависимости по частям речи
    pos_dependency_query = (
        ...
        .join(w, w.TokenID == tok.TokenID)
        .group_by(tok.Token_text, w.Part_of_speech)
    )

    # --- sentence_words: предложения с формами токена
    sentence_with_words_query = (
        select(
            tok.Token_text.label('Token_text'),
            s.Sentence_text
        )
        .join(cr, cr.TokenID == tok.TokenID)
        .join(s, s.SentenceID == cr.SentenceID)
        .distinct(tok.Token_text, s.SentenceID)
    )
...
def main():
    ...
    # <-- Новое: добавлена дисперсия (var_word_count)
    word_stats = (
        df_sw
    ...
    # <-- Новое: добавлено форматирование var_word_count
    df_final['avg_word_count']       = df_final['avg_word_count'].fillna(0).round(2)
    df_final['var_word_count']       = df_final['var_word_count'].fillna(0).round(2)  # <-- Дисперсия
    ...
    print("\nТОП-20 токенов:")
    print(expanded[['Token_text', 'avg_word_count', 'var_word_count', 'total_token_count']].head(20))  # <-- Вывод с дисперсией

    out_file = "generic_tokens_expanded_var.xlsx"
